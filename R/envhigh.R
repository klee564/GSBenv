...
#'
#'
env.high <- function(X, Y, u, asy = FALSE, init = NULL) {
  ...
  tmp <- envMU.high(M, U, u, initial = init)
  ...
  if (u == 0) {
    ...
    if (asy == T) {
      ...
      if(min(eigen(Omega0hat)$values)<1e-8){
        invOmega0hat <- PDSCE::pdsoft.cv(Omega0hat)$omega
      } else{invOmega0hat <- chol2inv(chol(Omega0hat))}
      ...
      if(min(eigen(temp)$values)<1e-8){
        invtemp <- PDSCE::pdsoft.cv(temp)$omega
      } else{invtemp <- chol2inv(chol(temp))}

      temp2 <- kronecker(t(etahat), Gamma0hat)
      covMatrix <- kronecker(invsigX, Sigma1) + temp2 %*% invtemp %*% t(temp2)
      asySE <- matrix(sqrt(diag(covMatrix)), nrow = r)
      ratio <- asyFm / asySE
    }
  }
...
envMU.high <- function(M, U, u, initial = NULL) {
